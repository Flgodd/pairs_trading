...
void scanLargeDeviceArray(double *d_out, double *d_in, int length, bool bcao, cudaStream_t stream) {
	...
	if (remainder == 0) {
		...
		scanLargeEvenDeviceArray(d_out, d_in, lengthMultiple, bcao, stream);
...
void scanLargeEvenDeviceArray(double *d_out, double *d_in, int length, bool bcao, cudaStream_t stream) {
	...
	if (bcao) {
		prescan_large<<<blocks, THREADS_PER_BLOCK, 2 * sharedMemArraySize, stream>>>(d_out, d_in, ELEMENTS_PER_BLOCK, d_sums);
	}
	else {
		prescan_large_unoptimized<<<blocks, THREADS_PER_BLOCK, 2 * sharedMemArraySize, stream>>>(d_out, d_in, ELEMENTS_PER_BLOCK, d_sums);
	}

	const int sumsArrThreadsNeeded = (blocks + 1) / 2;
	if (sumsArrThreadsNeeded > THREADS_PER_BLOCK) {
		// perform a large scan on the sums arr
		scanLargeDeviceArray(d_incr, d_sums, blocks, bcao, stream);
	}
	else {
